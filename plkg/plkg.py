...
def quantize_measurements(rssi_values: list, num_bits: int = 128) -> bytes:
    ...
    guard_band = 1.0  # dBm — values within ±1 of mean are discarded
...
if __name__ == "__main__":
    ...
    # Reconciliation
    syn_a = hashlib.sha256(bits_a + b"alice").digest()
    syn_b = hashlib.sha256(bits_b + b"bob").digest()
    
    key_a = information_reconciliation(bits_a, syn_b)
    key_b = information_reconciliation(bits_b, syn_a)
